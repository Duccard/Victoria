...
def search_royal_archives(query: str):
    """MANDATORY: Use for all facts. Searches Victorian PDFs and CSVs."""
    retriever = get_retriever()

    search_variations = [query]
    low_q = query.lower()
    if "steam" in low_q or "engine" in low_q:
        search_variations.append("James Watt 1769 steam engine GB913")
    if "loom" in low_q:
        search_variations.append("Cartwright power loom textiles")

    all_docs = []
    for var in search_variations:
        all_docs.extend(retriever.invoke(var))

    evidence_list, context_blocks, seen = [], [], set()
    ...
    for d in all_docs[:10]:
        ...
        if ref not in seen:
            ...
            seen.add(ref)
            context_blocks.append(f"SOURCE {title} (PG {page}):\n{d.page_content}")
...
with st.sidebar:
    ...
    st.session_state.current_style = st.selectbox(
        "Select Correspondent:", list(AVATARS.keys())[:-1]
    )
...
if "pending_input" in st.session_state and st.session_state.pending_input:
    ...
    persona_prompts = {
        "Queen Victoria": "You are Queen Victoria. Use the Royal 'We'. Be authoritative.",
        "Oscar Wilde": "You are Oscar Wilde. Be witty and flamboyant.",
        "Jack the Ripper": "Speak in a dark, menacing Whitechapel whisper.",
        "Isambard Kingdom Brunel": "Speak with engineering passion and technical precision.",
    }
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                f"{persona_prompts[st.session_state.current_style]}\n\nPROTOCOL:\n1. Call search_royal_archives immediately.\n2. Use the exact years/inventors from the tool output.\n3. Do not say info is missing if the tool provides it.",
            ),
    ...
    vic_agent = AgentExecutor(agent=agent, tools=tools, verbose=True)
    ...
    with st.chat_message("assistant", avatar=AVATARS[st.session_state.current_style]):
        with st.status("📖 Consulting Records...") as status:
            ...
            status.update(label="✅ Found information", state="complete")
